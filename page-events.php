...
$fe_id       = get_the_ID();
$fe_date_fmt = $fe_date ? date('l, F j, Y', strtotime($fe_date)) : '';
...
<section style="padding-top:72px;border-bottom:1px solid #1a1a1a;background:#080808;">
  <div style="display:grid;grid-template-columns:1fr 1fr;min-height:90vh;">
    ...
    <div style="position:relative;overflow:hidden;background:#0a0a0a;">
      <?php if (!empty($fe_flyer['url'])): ?>
        <img src="<?php echo esc_url($fe_flyer['url']); ?>"
             alt="<?php echo esc_attr(get_the_title($fe_id)); ?>"
      ...
      <?php else: ?>
        <div style="width:100%;height:100%;min-height:600px;background:linear-gradient(135deg,#1a0500,#2d0800);display:flex;align-items:center;justify-content:center;">
          <span style="font-family:'Bebas Neue',sans-serif;font-size:48px;color:rgba(255,69,0,0.15);letter-spacing:4px;text-align:center;padding:40px;"><?php echo esc_html(get_the_title($fe_id)); ?></span>
        </div>
      <?php endif; ?>
      <!-- Dark gradient over flyer bottom -->
    ...
    <div style="padding:80px 64px;display:flex;flex-direction:column;justify-content:center;position:relative;background:#080808;">
      ...
      <!-- Watermark year -->
      <div style="position:absolute;top:40px;right:40px;font-family:'Bebas Neue',sans-serif;font-size:100px;color:rgba(255,255,255,0.03);line-height:1;pointer-events:none;user-select:none;"><?php echo esc_html($fe_year); ?></div>
      ...
      <!-- Event Name -->
      <h1 style="font-family:'Bebas Neue',sans-serif;font-size:clamp(40px,5vw,68px);letter-spacing:2px;line-height:0.92;color:#fff;text-transform:uppercase;margin-bottom:32px;" class="ab-reveal">
        <?php echo esc_html(get_the_title($fe_id)); ?>
      </h1>

      <!-- Date Block -->
      <?php if ($fe_date): ?>
      <div style="display:flex;align-items:flex-start;gap:20px;margin-bottom:32px;padding:24px 0;border-top:1px solid #1a1a1a;border-bottom:1px solid #1a1a1a;" class="ab-reveal">
      ...
      <!-- Artists -->
      <?php if ($fe_artists): ?>
      <div style="margin-bottom:32px;" class="ab-reveal">
      ...
      <!-- PRIMARY CTA -->
      <div style="display:flex;flex-direction:column;gap:12px;" class="ab-reveal">
        <?php if ($fe_ticket && $fe_status !== 'sold_out'): ?>
          <a href="<?php echo esc_url($fe_ticket); ?>"
             target="_blank" rel="noopener"
             style="display:block;background:#FF4500;color:#fff;font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:700;letter-spacing:3px;text-transform:uppercase;padding:20px 40px;border-radius:2px;text-align:center;transition:background 0.2s;text-decoration:none;"
             onmouseover="this.style.background='#CC3600'" onmouseout="this.style.background='#FF4500'">
            Buy Tickets Now →
          </a>
          <a href="<?php echo esc_url(get_permalink($fe_id)); ?>"
             style="display:block;background:transparent;color:rgba(255,255,255,0.4);font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:600;letter-spacing:3px;text-transform:uppercase;padding:14px 40px;border-radius:2px;text-align:center;border:1px solid #1a1a1a;transition:color 0.2s,border-color 0.2s;text-decoration:none;"
        ...
        <?php elseif ($fe_status === 'sold_out'): ?>
          <div style="background:#1a1a1a;color:rgba(255,255,255,0.3);font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:700;letter-spacing:3px;text-transform:uppercase;padding:20px 40px;border-radius:2px;text-align:center;">
            Sold Out
          </div>
          <a href="<?php echo esc_url(get_permalink($fe_id)); ?>" style="display:block;text-align:center;font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:600;letter-spacing:3px;text-transform:uppercase;color:rgba(255,255,255,0.3);text-decoration:none;padding:14px;">Event Details →</a>
        <?php else: ?>
          <div style="background:#1a1a1a;color:rgba(255,255,255,0.3);font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:700;letter-spacing:3px;text-transform:uppercase;padding:20px 40px;border-radius:2px;text-align:center;">
            Tickets Coming Soon
          </div>
      ...
      <!-- Capacity badge -->
      <?php if ($fe_capacity): ?>
      <div style="margin-top:24px;display:flex;align-items:center;gap:8px;" class="ab-reveal">
...
<?php endif; // end featured event ?>

<!-- ═══ ALL EVENTS GRID ═══ -->
<section style="padding:80px 56px 120px;">

  <!-- Filter Tabs -->
  <div style="display:flex;gap:0;margin-bottom:56px;border-bottom:1px solid #1a1a1a;" class="ab-reveal">
    <?php foreach (['upcoming'=>'Upcoming','past'=>'Past Events'] as $key=>$label): ?>
      <a href="?filter=<?php echo esc_attr($key); ?>"
         style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:700;letter-spacing:3px;text-transform:uppercase;padding:16px 32px;border-bottom:2px solid <?php echo $filter===$key ? '#FF4500' : 'transparent'; ?>;color:<?php echo $filter===$key ? '#fff' : 'rgba(255,255,255,0.3)'; ?>;transition:color 0.2s;text-decoration:none;display:inline-block;">
  ...
  <!-- Section header -->
  <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:40px;" class="ab-reveal">
    <div>
  ...
  <?php
  // For upcoming — exclude the featured event already shown at top
    ...
    ?>
      <div class="ab-event-card ab-reveal">
        <div class="ab-event-img-wrap">
          ...
          <?php if ($status === 'sold_out'): ?><span class="ab-event-status-live" style="background:rgba(100,100,100,0.9);">Sold Out</span><?php endif; ?>
        </div>
...
<!-- Mobile responsive fix for featured event -->
<style>
